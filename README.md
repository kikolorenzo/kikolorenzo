![header](https://github.com/kikolorenzo/kikolorenzo/blob/main/banner%201.png)

<div align="center">
  
[![Typing SVG](https://readme-typing-svg.demolab.com?font=DejaVu+Sans&weight=600&size=25&duration=2800&pause=2000&color=FFFFFF&center=true&vCenter=true&repeat=true&width=435&lines=Hola%2C+soy+Kiko+Lorenzo&background=131723)](https://git.io/typing-svg)
</div>
<div align="center">
<h2> &nbsp; <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="40px">
</div>
    
<br>

**Sobre mi...**

Soy desarrollador full stack enfocado en crear aplicaciones web modernas y eficientes.
Me interesa especialmente integrar herramientas de IA para optimizar el proceso de desarrollo
y mejorar la calidad del código. Con cada proyecto, busco implementar soluciones que combinen
las mejores prácticas de desarrollo con la asistencia de IA para lograr resultados más efectivos.

Valoro la colaboración y el aprendizaje continuo. Me motiva explorar cómo las nuevas tecnologías de IA
pueden mejorar el flujo de trabajo en desarrollo web. Siempre abierto a nuevas herramientas y metodologías
que impulsen la productividad y la calidad del código.
<br><br>

## 💻 &nbsp; Stack Tecnológico

<table align="center">
<br/>

<!-- Frontend -->
<div align="center">
  <h4>🪟 &nbsp;Frontend</h4>
  <img style="margin: 10px" src="https://img.shields.io/badge/html5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white" height="30" />
  <img style="margin: 10px" src="https://img.shields.io/badge/css3-%231572B6.svg?style=for-the-badge&logo=css3&logoColor=white" height="30" />
  <img style="margin: 10px" src="https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E" height="30" />
  <img style="margin: 10px" src="https://img.shields.io/badge/typescript-%23007ACC.svg?style=for-the-badge&logo=typescript&logoColor=white" height="30" />
  <img style="margin: 10px" src="https://img.shields.io/badge/react-%2320232a.svg?style=for-the-badge&logo=react&logoColor=%2361DAFB" height="30" />
  <img style="margin: 10px" src="https://img.shields.io/badge/astro-%23000000.svg?style=for-the-badge&logo=astro&logoColor=white" height="30" />
  <img style="margin: 10px" src="https://img.shields.io/badge/angular-DD0031?style=for-the-badge&logo=angular&logoColor=white" height="30" />
  <img style="margin: 10px" src="https://img.shields.io/badge/vue.js-35495E?style=for-the-badge&logo=vue.js&logoColor=4FC08D" height="30" />
</div>
<br/>

<!-- Backend -->
<div align="center">
  <h4>🖧 &nbsp;Backend</h4>
  <img style="margin: 10px" src="https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54" height="30" />
  <img style="margin: 10px" src="https://img.shields.io/badge/node.js-6DA55F?style=for-the-badge&logo=node.js&logoColor=white" height="30" />
  <img style="margin: 10px" src="https://img.shields.io/badge/django-092E20.svg?style=for-the-badge&logo=django&logoColor=white" height="30" />
  <img style="margin: 10px" src="https://img.shields.io/badge/java-%23ED8B00.svg?style=for-the-badge&logo=openjdk&logoColor=white" height="30" />
  <img style="margin: 10px" src="https://img.shields.io/badge/rust-%23000000.svg?style=for-the-badge&logo=rust&logoColor=white" height="30" />
</div>
<br/>

<!-- Bases de Datos -->
<div align="center">
  <h4>🗄️ &nbsp;Bases de Datos</h4>
  <img style="margin: 10px" src="https://img.shields.io/badge/sql-%2300758F.svg?style=for-the-badge&logo=sql&logoColor=white" height="30" />
</div>
<br/>

<!-- Herramientas y CMS -->
<div align="center">
  <h4>🛠️ &nbsp;Herramientas y CMS</h4>
  <img style="margin: 10px" src="https://img.shields.io/badge/wordpress-%2321759b.svg?style=for-the-badge&logo=wordpress&logoColor=white" height="30" />
</div>
<br/>
</table>

<br/>

## 🤖💡 &nbsp; IA en mi Desarrollo Full Stack
<br/><br/>

<h4>CodeBuddy: Asistente de programación con IA</h4>


CodeBuddy es una plataforma web que mejora la productividad del desarrollador. Integra GPT-3 para ofrecer
sugerencias de código y explicaciones detalladas. La IA aceleró el desarrollo, permitiendo:

- Implementar NLP avanzado para análisis de código   <img align="right" alt="GIF" src="https://github.com/abhisheknaiidu/abhisheknaiidu/blob/master/code.gif?raw=true" width="40%"/>
- Crear un dashboard interactivo con métricas complejas
- Desarrollar un sistema de detección de bugs basado en ML
<br/><br/>

<h4>TravelBudget: Planificador de viajes inteligente</h4>

TravelBudget optimiza presupuestos de viaje utilizando aprendizaje automático. La IA facilitó la implementación
de algoritmos complejos, logrando:

- Predicción precisa de precios de vuelos y alojamientos
- Generación de itinerarios personalizados
- Visualización en tiempo real de datos de gastos y ahorros
<br/><br/>

Estos proyectos demuestran cómo la IA potencia el desarrollo web, creando soluciones innovadoras y eficientes.
<br/><br/>

## Contáctame 📫

<br/> [![Gmail](https://img.shields.io/badge/kikolorenzodfs@gmail.com-white?style=for-the-badge&logo=gmail&logoColor=red&label=email%20de%20contacto&labelColor=white&color=yellowgreen)](mailto:kikolorenzodfs@gmail.com)

<br/> <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="25px"> &nbsp; **¡Gracias por tu visita!**

<br>

 &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; ![Visitas al perfil](https://komarev.com/ghpvc/?username=kikolorenzo&color=blue)

<br>

<div align="center">
  <h2 style="margin: 5px 10px;">Github Stats:</h2>
  <p align="center">
    <a href="https://github.com/kikolorenzo">
      <img src="https://github-readme-stats.vercel.app/api?username=kikolorenzo&show_icons=true&theme=dark&hide_border=true&locale=en" alt="GitHub Stats" height="180em" />
    </a>
    <a href="https://github.com/kikolorenzo">
      <img src="https://github-readme-streak-stats.herokuapp.com/?user=kikolorenzo&theme=dark&hide_border=true" alt="GitHub Streak Stats" height="180em" />
    </a>
  </p>
</div>
